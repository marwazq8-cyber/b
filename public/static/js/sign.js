const key='722e44eb1e83491475ad0b2de04b82ca';
const myJs = {
    /**
     * 生成json
     * @param url 要加载的脚本路径
    ...
    getJson: function (list) {
        ...
        let querys = this.ksort(this, all);
        querys['key'] = key;
        let sign_str = '';
        for (const q in querys) {
            sign_str += q + '=' + querys[q] + '&';
        }
        const http_param_str = sign_str.substring(0, sign_str.length - 1);
        return $.md5(http_param_str);
    },
    /**
     * javascript实现PHP字典排序
