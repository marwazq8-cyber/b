var alignmentFns = {
    ...
    init: function(input, step, max, min, digit) {
        ...
        //添加左右加减号
        input.wrap("<div id = '" + input.attr('id') + "div' style = 'overflow:hidden;width:" + width + "px;height:" + height + "px;border: none;background:#fa7cb7;border-radius:100px;color:#fff;margin:0 auto'></div>");
        input.before("<div id = '" + input.attr('id') + "l'  onselectstart = 'return false;' style = '-moz-user-select:none;cursor:pointer;text-align:center;width:" + height + "px;height:" + height + "px;line-height:" + height + "px;border-right: none;float:left'>-</div>");
        input.after("<div id = '" + input.attr('id') + "r'  onselectstart = 'return false;' style = '-moz-user-select:none;cursor:pointer;text-align:center;width:" + height + "px;height:" + height + "px;line-height:" + height + "px;border-left: none;float:right'> + </div>");
        //左右调用执行
