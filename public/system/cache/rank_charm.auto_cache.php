...
class rank_charm_auto_cache
{
    private $key = "rank:charm";
            ...
            if(!$is_ok){
                ...
                if($rank_name=='day'){//day
                    ...
                    $list = db('user_consume_log')
                        -> alias('c')
                        -> join($db_prefix . 'user u','c.to_user_id=u.id')
                        -> field('u.id,u.user_nickname,u.sex,u.avatar,u.address,u.is_online,sum(c.profit) as total')
                        -> group('c.to_user_id')
