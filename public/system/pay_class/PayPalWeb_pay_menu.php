...
class PayPalWeb_pay
{
    ...
    public function get_payment_code($pay_type, $rule, $payment_notice_id)
    {
        ...
        $url =  SITE_URL . "/api/paypal_web_api/pay_index?pay_code=PayPalWeb&notice_id=". $payment_notice_id."&user_id=".$uid."&token=".$token;
        $pay_info = array();
        $pay_info['is_wap'] = 1;
        $pay_info['class_name'] = "PayPalWeb";
        $pay_info['is_without'] = 0; //
        $pay_info['url'] = $url;
        $pay_info['type'] = 1;
        $pay_info['sdk_code'] = array("pay_sdk_type" => "PayPal", "config" =>
            array(
                "url" => $url,
                "is_wap" => 1,
            )
