        ...
        if (!empty($storage)) {
            ...
            if (!empty($storage['type'])) {
                ...
                if ($storage['type'] == 'Tencent') {
                    ...
                    $arrInfo["file_name"] = 'admin/' . $fileImage->getFileName();

                    $arrInfo["file_path"] = $strWebPath . $fileSaveName;
                    $arrInfo["suffix"] = $fileImage->getExtension();


                    $res = $this->qcloud_upload($arrInfo);
                    if (!$res) {
                        return false;
                    }

                    //dump($res);die();
                    //dump($strWebPath . $fileSaveName);die();
                    return [
                        'filepath'    => $res,
                        "name"        => $arrInfo["file_name"],
                        'id'          => $strId,
                        'preview_url' => $res,
                        'url'         => $res,
                    ];
