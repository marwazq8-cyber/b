...
class LangSave extends Command
{
    protected function configure()
    {
        $this->setName('lang')
            ->addArgument('name', Argument::OPTIONAL, "键名")
            ->setDescription('山东布谷鸟网络科技[提取语言包]');
    }
    protected function traverse($path){
        ...
        while(($file = readdir($currentDir)) !== false) {
            ...
            //构建子目录路径
            if($file == '.' || $file == '..' || $file == 'config.php' || $file == 'database.php' || $file =='validate') {
                continue;
    ...
    protected function execute(Input $input, Output $output)
    {
        try {
            ...
            $dir = DOCUMENT_ROOT."/application/admin";
            ...
            $lang_files = require_once $lang_dir;
            $dir = DOCUMENT_ROOT."/application/user/controller";
