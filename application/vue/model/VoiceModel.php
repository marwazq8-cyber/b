...
class VoiceModel extends Model
{
    ...
    */
    public function sel_voice_one($uid)
    {
        ...
        // 查询语音房间
        $field = "a.user_nickname,a.avatar,a.host_more_voice_ratio,v.title,v.id,v.group_id,v.user_id,v.voice_bg,v.wheat_type,v.announcement,v.avatar as voice_avatar,v.voice_type,a.luck,v.live_in,v.type,v.online_number,v.online_count,v.room_id,v.voice_label,a.vip_end_time";
   ...
   */
    public function upd_cumulative($uid, $name, $number, $type)
    {
        ...
        if ($type == 1) {
            ...
            // 累减
            $voice_status = db('voice')->where("user_id=" . $uid)->setDec($name, $number);
        }
    ...
    */
    ...
    public function add_voice_vote_number($id, $vote_number)
    {
        ...
        $voice = db('voice')->where("id=" . $id)->setinc('vote_number', $vote_number);
   ...
   */
    ...
    */
    ...
    /* 选择房间类型 */
    public function sel_voice_type($type)
    {

        $voice_type = db('voice_type')->field("id,name,type")->where("status=1 and type=" . $type)->order("sort desc")->select();
    ...
    public function get_voice_administrator_user($voice_id, $uid)
    {
        ...
        $list = db('user')->alias('a')
            ...
            ->field("a.user_nickname,a.avatar,v.*,a.vip_end_time")
            ->where("v.voice_id=" . $voice_id . " and v.voice_uid=" . $uid)
            ->select();
        foreach ($list as &$v) {
            // 是否是vip
            $v['is_vip'] = get_is_vip($v['vip_end_time']);
    ...
    public function del_voice_administrator($voice_id, $touid, $uid)
    {
        ...
        $upd_voice = db('voice_administrator')->where("voice_id=" . $voice_id . " and user_id=" . $touid . " and voice_uid=" . $uid)->delete();
    ...
    // 发布我的录音库
    public function add_my_voice_bank($data)
    {

        $atte = db('voice_bank')->insert($data);

        return $atte;
    }

    // 获取对方发布的录音
    public function voice_bank_list($where, $page)
    {
        $list = db('voice_bank')
            ->where($where)
            ->field("id,uid,title,img,path,label,addtime,status")
            ->order("addtime DESC")
            ->page($page)
            ->select();
    ...
    // 获取我发布的录音
    public function my_voice_bank_list($where)
    {
        $list = db('voice_bank')
            ->where($where)
            ->field("id,uid,title,img,path,label,addtime,status")
    ...
    // 获取发布一个录音信息
    public function voice_bank_one($where)
    {
        $list = db('voice_bank')
            ->where($where)
            ->field("id,uid,title,img,path,label,addtime")
            ->find();
    ...
    // 获取是否喜欢本录音
    public function voice_bank_like_one($where)
    {

        $list = db('voice_bank_like')->where($where)->find();

        return $list;
    }

    // 统计喜欢录音的数量
    public function voice_bank_like_count($where)
    {

        $list = db('voice_bank_like')->where($where)->count();

        return $list;
    }

    // 添加喜欢的录音
    public function add_voice_bank_like($data)
    {

        $atte = db('voice_bank_like')->insert($data);

        return $atte;
    }

    // 取消喜欢的录音
    public function del_voice_bank_like($where)
    {

        $status = db('voice_bank_like')->where($where)->delete();

        return $status;
    }

    // 撤销我发布的录音
    public function update_my_voice_bank($where, $data)
    {

        $status = db('voice_bank')->where($where)->update($data);

        return $status;
    }

    // 撤销我发布的录音
    public function del_my_voice_bank($where)
    {

        $status = db('voice_bank')->where($where)->delete();
