...
class RbacController extends AdminBaseController
{
    ...
    public function roleAddPost()
    {
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                if ($result) {
                    $this->success(lang('Role_added_successfully'), url("rbac/index"));
                } else {
                    $this->error(lang('Adding_role_failed'));
                }
    ...
    public function roleedit()
    {
        ...
        if ($id == 1) {
            $this->error(lang('super_administrator_role_cannot_modified'));
        }
        $data = Db::name('role')->where(["id" => $id])->find();
        if (!$data) {
            $this->error(lang('role_does_not_exist'));
        }
    ...
    public function roleDelete()
    {
        $id = $this->request->param("id", 0, 'intval');
        if ($id == 1) {
            $this->error(lang('super_administrator_role_cannot_deleted'));
        }
        $count = Db::name('RoleUser')->where(['role_id' => $id])->count();
        if ($count > 0) {
            $this->error(lang('role_already_has_users'));
        } else {
    ...
    public function authorize()
    {
        ...
        foreach ($result as $n => $t) {
            $name       = $t['app'];
            $controller = ucwords($t['controller']);
            $action     = $t['action'];
            $name_lang = strtoupper($name . '_' . $controller . '_' . $action);
    ...
    public function authorizePost()
    {
        if ($this->request->isPost()) {
            ...
            if (!$roleId) {
                $this->error(lang('role_requiring_authorization_does_not_exist'));
            }
            if (is_array($this->request->param('menuId/a')) && count($this->request->param('menuId/a')) > 0) {
                ...
                $this->success(lang('Authorization_succeeded'));
            } else {
                //当没有数据时，清除当前角色授权
                Db::name("authAccess")->where(["role_id" => $roleId])->delete();
                $this->error(lang('Clearing_authorization_succeeded'));
            }
        }
    }
