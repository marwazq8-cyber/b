...
class NavMenuController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        if (empty($intNavId)) {
            $this->error(lang('Please_specify_navigation'));
        }
        ...
        foreach ($arrResult as $r) {
            $r['str_manage'] = '<a href="' . url("NavMenu/add", ["parent_id" => $r['id'], "nav_id" => $r['nav_id']]) . '">{:lang(\'ADD_SUB_MENU\')}</a>  <a href="'
                . url("NavMenu/edit", ["id" => $r['id'], "parent_id" => $r['parent_id'], "nav_id" => $r['nav_id']]) . '">{:lang(\'EDIT\')}</a>  <a class="js-ajax-delete" href="' . url("NavMenu/delete", ["id" => $r['id'], 'nav_id' => $r['nav_id']]) . '">{:lang(\'DELETE\')}</a> ';
