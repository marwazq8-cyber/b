...
class PluginController extends AdminBaseController
{
    ...
    public function toggle()
    {
        ...
        if (empty($pluginModel)) {
            $this->error(lang('Plug_in_does_not_exist'));
        }
    ...
    public function setting()
    {
        ...
        if (empty($plugin)) {
            $this->error(lang('Plug_in_not_installed'));
        }
...
//            fclose($fp);
    ...
    public function install()
    {
        ...
        if ($pluginCount > 0) {
            $this->error(lang('Plug_in_installed'));
        }

        $plugin = new $class;
        $info   = $plugin->info;
        if (!$info || !$plugin->checkInfo()) {//检测信息的正确性
            $this->error(lang('Missing_plug_in_information'));
        }

        $installSuccess = $plugin->install();
        if (!$installSuccess) {
            $this->error(lang('Plug_in_preinstallation_failed'));
        }
        ...
        $this->success(lang('Installation_succeeded'));
    }
    ...
    public function uninstall()
    {
        ...
        $this->success(lang('Uninstall_succeeded'));
    }
