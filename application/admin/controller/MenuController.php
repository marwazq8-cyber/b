...
class MenuController extends AdminBaseController
{
    ...
    {
        if(OPEN_ADMIN_MENU != 1){
            $this->error(lang('Adding_without_permission_failed'));exit;
        }
        if ($this->request->isPost()) {
            $result = $this->validate($this->request->param(), 'AdminMenuValidate');
            if ($result !== true) {
                $this->error($result);
            } else {
    ...
    {
        if(OPEN_ADMIN_MENU != 1){
            $this->error(lang('Modification_failed'));exit;
        }
    ...
    {
        if(OPEN_ADMIN_MENU != 1){
            $this->error(lang('Delete_without_permission_failed'));exit;
        }
        $id    = $this->request->param("id", 0, 'intval');
        $count = Db::name('AdminMenu')->where(["parent_id" => $id])->count();
        if ($count > 0) {
            $this->error(lang('There_submenus_that_cannot_deleted'));
        }
        if (Db::name('AdminMenu')->delete($id) !== false) {
            $this->success(lang('Menu_deleted_successfully'));
        } else {
    ...
    public function getActions()
    {
        ...
        if (!in_array($app, $apps)) {
            $this->error( $app . lang('model data Not Found'));
        }
