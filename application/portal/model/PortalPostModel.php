...
class PortalPostModel extends Model
{
    ...
    {
        return htmlspecialchars(cmf_replace_content_file_url(htmlspecialchars_decode($value), false));
    }
    ...
    public function adminAddArticle($data, $categories)
    {
        ...
        Db::name('portal_post')->where(['id' => $this->id])->update([
            'published_time' =>  $data['post_status'] == 1 ? time() : 0
        ]);
        return $this;
    ...
    public function adminEditArticle($data, $categories)
    {
        ...
        Db::name('portal_post')->where(['id' => $data['id']])->update([
            'published_time' =>  $data['post_status'] == 1 ? time() : 0
        ]);
        return $this;
