<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:47:"themes/admin_simpleboot3/admin/index/index.html";i:1730282264;}*/ ?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        .admin_message_index{width:100%;height:25px;line-height: 25px;text-align: center;display: block;}
        .admin_message_index:hover{text-decoration:none;color: #C20C0C;}
        .admin_message_index_div{margin-top:15px;}
    ...
    <?php $submenus=$menus; function getsubmenu($submenus){ if(!(empty($submenus) || (($submenus instanceof \think\Collection || $submenus instanceof \think\Paginator ) && $submenus->isEmpty()))): foreach($submenus as $menu){ ?>
        <li>
            ...
            <a href="#" class="dropdown-toggle">
                ...
                <span class="menu-text back"><?php echo lang('BACK'); ?></span>
...
<div id="right-tools-wrapper">
    <!--<span id="right_tools_clearcache" title="<?php echo lang('清除缓存'); ?>" onclick="javascript:openapp('<?php echo url('admin/Setting/clearcache'); ?>','right_tool_clearcache','清除缓存');"><i class="fa fa-trash-o right_tool_icon"></i></span>-->
...
<div class="navbar navbar-default">
    <div class="container-fluid">
        <div class="navbar-header">
            <a href="<?php echo url('admin/index/index'); ?>" class="navbar-brand" style="min-width: 200px;text-align: center;"><?php echo $config_log['system_name']; ?></a>
            ...
            <div class="pull-left" style="position: relative;">
                ...
                <div id="task-content">
                    <ul class="nav navbar-nav cmf-component-tab" id="task-content-inner">
                        <li class="cmf-component-tabitem noclose" app-id="0" app-url="<?php echo url($home_url); ?>"
                            app-name="<?php echo lang('首页'); ?>">
            ...
            <ul class="nav navbar-nav navbar-right simplewind-nav">
                <?php if($debug): ?>
                    <li class="light-blue" style="border-left:none;">
                        <!--                        <a  href="<?php echo url('user/clear_data'); ?>"  style="color:#fff;font-size: 16px">-->
                        <!--                           清除系统所有数据（不明白意思的不要点击）-->
                        <!--                        </a>-->
                    </li>
                <?php endif; ?>
<!--                <li class="light-blue" style="border-left:none;">-->
<!--                    <select name="status" style="width: 100px;height: 35px;text-align: center;" class="lang_btn">-->
<!--                        <?php if(is_array($lang_list) || $lang_list instanceof \think\Collection || $lang_list instanceof \think\Paginator): if( count($lang_list)==0 ) : echo "" ;else: foreach($lang_list as $key=>$l): ?>-->
<!--                            <option value="<?php echo $l['id']; ?>" <?php if($lang == $l['id']): ?> selected=selected <?php endif; ?> ><?php echo $l['name']; ?></option>-->
                ...
                <li class="light-blue" style="border-left:none;">
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu" style="min-width: 100px!important;">
                            <?php if(is_array($lang_list) || $lang_list instanceof \think\Collection || $lang_list instanceof \think\Paginator): if( count($lang_list)==0 ) : echo "" ;else: foreach($lang_list as $key=>$l): ?>
                                <li><a href="javascript:void(0)" class="lang_btn" data-id="<?php echo $l['id']; ?>"><?php echo $l['name']; ?></a></li>
                            <?php endforeach; endif; else: echo "" ;endif; ?>
                        </ul>
                    </div>
                </li>
                ...
                <li class="light-blue dropdown" style="border-left:none;">
                    ...
                    <ul class="user-menu pull-right dropdown-menu dropdown-yellow dropdown-caret dropdown-closer">
                        <!--   <?php if(cmf_auth_check(cmf_get_current_admin_id(),'admin/Setting/site')): ?>
                          ...
                          <?php endif; ?> -->
                        <?php if(cmf_auth_check(cmf_get_current_admin_id(),'admin/Setting/password')): ?>
                            <li>
...
<div class="main-container container-fluid">
    ...
    <div class="sidebar" id="sidebar">
        <div class="sidebar-shortcuts" id="sidebar-shortcuts">
            ...
            <?php if(cmf_auth_check(cmf_get_current_admin_id(),'portal/AdminCategory/index')): ?>
                <a class="btn btn-sm btn-success" href="javascript:openapp('<?php echo url('portal/AdminCategory/index'); ?>','index_termlist','文章分类管理');" title="<?php echo lang('文章分类管理'); ?>">
                    <i class="fa fa-th"></i>
                </a>
            <?php endif; if(cmf_auth_check(cmf_get_current_admin_id(),'portal/AdminArticle/index')): ?>
                <a class="btn btn-sm btn-info" href="javascript:openapp('<?php echo url('portal/AdminArticle/index'); ?>','index_postlist','文章管理');" title="<?php echo lang('文章管理'); ?>">
    ...
    <div class="main-content">
        <div class="page-content" id="content">
            <iframe src="<?php echo url($home_url); ?>" style="width:100%;height: 100%;" frameborder="0" id="appiframe-0"
...
<script>
    $(function () {
        ...
        // get_message();
        // setInterval("get_message();",1000*60);
    });
    $(".lang_btn").click(function (){
        const lang_val = $(this).attr('data-id');
        $.ajax({
            url: "<?php echo url('admin/index/lang_save'); ?>",
            type: 'get',
            dataType: 'json',
    ...
    function get_message() {
        //右下角弹出未读的消息
        $.ajax({
            url: "<?php echo url('index/message_index'); ?>",
            type: 'post',
            dataType: 'json',
            success: function (data) {
                console.log(data);
                var html='<div class="admin_message_index_div">';
                if(data.auth_record >0){
                    html+='<a class="admin_message_index" href="javascript:openapp(\'/admin/public/User/identity/auth_info_list.html\')"><?php echo lang("认证未审核：'+data.auth_record+'条"); ?></a>';
                }
                if(data.user_img >0){
                    html+='<a class="admin_message_index" href="javascript:openapp(\'/admin/public/Admin/audit/user_thumb.html\')"><?php echo lang("封面未审核：'+data.user_img+'条"); ?></a>';
                }
                if(data.user_pictures >0){
                    html+='<a class="admin_message_index" href="javascript:openapp(\'/admin/public/Admin/audit/photos.html\')"><?php echo lang("私照未审核：'+data.user_pictures+'条"); ?></a>';
                }
                if(data.user_video >0){
                    html+='<a class="admin_message_index" href="javascript:openapp(\'/admin/public/Admin/audit/index.html\')"><?php echo lang("视频未审核：'+data.user_video+'条"); ?></a>';
                }
